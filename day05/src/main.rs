...
fn main() {
    ...
    let seeds = number_regex
        ...
        .map(|c| c[0].parse::<u32>().unwrap_or(0))
        .collect::<Vec<u32>>();

    let mut seed_to_soil_map: HashMap<u32, u32> = HashMap::new();
    let mut soil_to_fertilizer_map: HashMap<u32, u32> = HashMap::new();
    let mut fertilizer_to_water_map: HashMap<u32, u32> = HashMap::new();
    let mut water_to_light_map: HashMap<u32, u32> = HashMap::new();
    let mut light_to_temperature_map: HashMap<u32, u32> = HashMap::new();
    let mut temperature_to_humidity_map: HashMap<u32, u32> = HashMap::new();
    let mut humidity_to_location_map: HashMap<u32, u32> = HashMap::new();
    ...
    for line in lines {
        ...
        for i in 0..range {
            current_map.insert(source_num + i, target_num + i);
        }
    }
    ...
    // println!("{}", lines.next().unwrap_or("Nothing!"));

    for seed in seeds {
        let mut location: &u32 = &seed;
        println!("Starting at {}", location);
        for map in &mut all_maps {
            location = map.get(location).unwrap_or(location);
        }
